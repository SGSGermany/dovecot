...
log() {
    printf "%s %s: Info: %s\n" "$(date +'%b %d %H:%M:%S')" "dovecot-sql-setup" "$(printf "$@")" >&2
}
...
if [ -z "$(find /etc/dovecot -name 'dovecot-sql*.conf.ext' -mindepth 1 -maxdepth 1 -print -quit)" ]; then
    exit
fi

log "Setting up Dovecot SQL configuration"
...
for FILE in /etc/dovecot/dovecot-sql*.conf.ext; do
