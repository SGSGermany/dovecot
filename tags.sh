...
echo + "VERSION=\"\$(podman run -i --rm $(quote "$IMAGE_ID")" \
    "apk search --no-cache --exact dovecot" \
    "| sed -ne 's/^dovecot-\(.*\)$/\1/p')\"" >&2

VERSION="$(podman run -i --rm "$IMAGE_ID" \
    apk search --no-cache --exact dovecot \
    | sed -ne 's/^dovecot-\(.*\)$/\1/p')"

if [ -z "$VERSION" ]; then
    echo "Unable to read version of the 'dovecot' Alpine package: Package not found" >&2
    exit 1
elif ! [[ "$VERSION" =~ ^([0-9]+)\.([0-9]+)\.([0-9]+)(\.([0-9]+))?([+~-]|$) ]]; then
    echo "Unable to read version of the 'dovecot' Alpine package: '$VERSION' is no valid version" >&2
    exit 1
fi

VERSION="${BASH_REMATCH[1]}.${BASH_REMATCH[2]}.${BASH_REMATCH[3]}.${BASH_REMATCH[5]:-0}"
